...
all:
	...
	sudo chmod 647 /opt/nuoj/nuoj_service.py
	sudo chmod 647 /opt/nuoj/nuoj_judger.py
	# clone service file
	...
	sudo mysql --user="root" --database="NuOJ" --execute="CREATE TABLE \`problem\` (\`problem_id\` int NOT NULL AUTO_INCREMENT, \`problem_name\` VARCHAR(20) NOT NULL, \`problem_deadline\` VARCHAR(20) NOT NULL, \`problem_author\` VARCHAR(20) NOT NULL, PRIMARY KEY(problem_id));"
	# create a new admin account
	sudo mysql --user="root" --database="NuOJ" --execute="INSERT INTO \`user\` (username, password, email, admin) VALUES ('NuOJ', 'Nu0J!@#$$', 'NuOJ@ntut.edu.tw', 1);"
	# install pip package
