...
def session_veri():
    ...
    return Response(
        json.dumps({"status": "OK", "handle": jwt_decode(SID)["handle"]}),
        mimetype="application/json",
    )
...
def returnLoginPage():
    ...
    if result["status"] == "OK":
        ...
        resp.set_cookie(
            "SID",
            value=sessionID,
            expires=(datetime.now(tz=timezone.utc) + timedelta(days=1)).timestamp(),
        )
...
def mail_verification():
    ...
    database_util.command_execute(
        "UPDATE `user` SET `email_verified`=1 WHERE handle=%s", (handle)
    )
