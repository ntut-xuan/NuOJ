...
def returnProblemIDPage(ID):
    problem_pid = database_util.command_execute(
        "SELECT problem_pid FROM `problem` WHERE `ID` = %s", (ID)
    )[0]["problem_pid"]
    problemJsonObject = json.loads(
        database_util.file_storage_tunnel_read(
            problem_pid + ".json", TunnelCode.PROBLEM
        )
    )
...
def submitCode():
    try:
        ...
        user_uid = database_util.command_execute(
            "SELECT user_uid FROM `user` WHERE email=%s", (session_data["email"])
        )[0]["user_uid"]
        language = "C++"
        date = datetime.now()
        type = "NS"
        ...
        # Check problem exist
        problem_count = database_util.command_execute(
            "SELECT COUNT(*) FROM `problem` WHERE ID=%s", (data["problem_id"])
        )[0]["COUNT(*)"]

        if problem_count == 0:
            return Response(
                json.dumps(error_dict(ErrorCode.INVALID_DATA)),
                mimetype="application/json",
            )

        # Save code to storage
        database_util.file_storage_tunnel_write(
            submission_id + ".cpp", code, TunnelCode.SUBMISSION
        )

        # Save info to SQL
        database_util.command_execute(
            "INSERT INTO `submission`(solution_id, problem_id, user_uid, language, date, type, result) VALUE(%s, %s, %s, %s, %s, %s, %s)",
            (submission_id, problem_id, user_uid, language, date, type, "Pending"),
        )

        # fetch solution code
        solution_group = database_util.command_execute(
            "SELECT solution_group FROM `problem` WHERE ID=%s", (problem_id)
        )[0]["solution_group"]
        solutions = database_util.command_execute(
            "SELECT * FROM `submission` WHERE solution_group=%s", (solution_group)
        )
        ...
        if main_solution_id == None:
            return Response(
                json.dumps(error_dict(ErrorCode.UNEXCEPT_ERROR, "No valid solution.")),
                mimetype="application/json",
            )

        main_solution = database_util.file_storage_tunnel_read(
            main_solution_id + ".cpp", TunnelCode.SOLUTION
        )

        # fetch checker code (temp)
        checker = open("/etc/nuoj/example_code/temp_checker.cpp", "r").read()

        webhook_url = "https://nuoj.ntut-xuan.net/judge_result_webhook/%s/" % (
            submission_id
        )
        requests.post(
            "http://localhost:4439/judge",
            data=json.dumps(
                {
                    "code": code,
        ...
        return Response(
            json.dumps(error_dict(ErrorCode.UNEXCEPT_ERROR, str(e))),
            mimetype="application/json",
            status=500,
        )
...
def result_webhook(submission_uuid):
    ...
    database_util.command_execute(
        "UPDATE submission SET result=%s, time=%s, memory=%s WHERE solution_id=%s",
        (verdict, "%.3f" % float(average_time), average_memory, submission_uuid),
    )
...
def problemTestcaseSubmit():
    ...
    if ("problem_pid" not in data) or ("chunk" not in data) or ("hash" not in data):
        return Response(
            json.dumps(error_dict(ErrorCode.INVALID_DATA)), mimetype="application/json"
        )
    ...
    if result != testcase_hash:
        return Response(
            json.dumps(error_dict(ErrorCode.UPLOAD_FAILED, "Hash not match.")),
            mimetype="application/json",
            status=400,
        )
    ...
    count = database_util.command_execute(
        "SELECT COUNT(*) FROM `problem` WHERE problem_pid=%s", (problem_pid)
    )[0]["COUNT(*)"]
    if count == 0:
        return Response(
            json.dumps(error_dict(ErrorCode.INVALID_DATA, "Problem PID not exist")),
            mimetype="application/json",
            status=400,
        )
    ...
    if resp.status_code != 200:
        return Response(
            json.dumps(
                error_dict(
                    ErrorCode.UNEXCEPT_ERROR,
                    "sandbox return status code " + str(resp.status_code),
                )
            ),
            mimetype="application/json",
            status=400,
...
def getAllProblemList():
    ...
    problems = database_util.command_execute(
        "select * from problem limit %s offset %s;", (number_of_problem, offset)
    )
    result = []
    i = 0
...
def returnProblemPage():
    ...
    for i in range(len(problem_db)):
        raw_problem_data = database_util.file_storage_tunnel_read(
            problem_db[i]["problem_pid"] + ".json", TunnelCode.PROBLEM
        )
