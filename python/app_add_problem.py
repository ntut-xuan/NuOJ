...
def session_name_auth(func):
    ...
    def decorator(PID):
        ...
        problem_mysql_data = database_util.command_execute(
            "SELECT * from `problem` WHERE problem_pid = %s", (PID)
        )[0]
        problem_storage_raw_data = database_util.file_storage_tunnel_read(
            "%s.json" % PID, TunnelCode.PROBLEM
        )
...
def returnAddProblemPage():
    ...
    database_util.command_execute(
        "INSERT INTO `problem`(problem_pid, problem_author) VALUES(%s,%s)",
        (problem_pid, handle),
    )
...
def returnEditProblemPage(PID):
    ...
    database_util.file_storage_tunnel_write(
        "%s.json" % (PID), json.dumps(problem_data), TunnelCode.PROBLEM
    )
...
def pre_compile_and_save(PID):
    ...
    database_util.command_execute(
        "UPDATE problem SET solution_group=%s WHERE problem_pid=%s",
        (solution_group, problem_id),
    )
    ...
    for code_data in json_post_data["data"]:
        ...
        # Register into SQL
        database_util.command_execute(
            "INSERT `submission`(solution_id, problem_id, user_uid, language, date, type, solution_group) VALUES(%s, %s, %s, %s, %s, %s, %s)",
            (
                submission_uuid,
                problem_id,
                user_uid,
        ...
        database_util.file_storage_tunnel_write(
            submission_uuid + ".cpp", code_data["code"], TunnelCode.SOLUTION
        )
        # Doing POST to sandbox
        webhook_url = "https://nuoj.ntut-xuan.net/compile_result_webhook/%s/" % (
            submission_uuid
        )
        requests.post(
...
def fetch_solution(PID):
    ...
    solution_group = database_util.command_execute(
        "SELECT solution_group from `problem` WHERE problem_pid=%s", (PID)
    )[0]["solution_group"]
    # fetch submissions status
    submissions = database_util.command_execute(
    ...
    for submission in submissions:
        ...
        code = database_util.file_storage_tunnel_read(
            solution_id + ".cpp", TunnelCode.SOLUTION
        )
        result = database_util.command_execute(
            "SELECT result FROM `submission` WHERE solution_id=%s", (solution_id)
        )[0]["result"]
        if result == "" or result == None:
            complie_status = "Running"
        solution_group_data.append(
            {"code": code, "result": result, "uuid": solution_id}
        )
    return Response(
        json.dumps(
            {
                "status": "OK",
                "compile_status": complie_status,
                "data": solution_group_data,
            }
        ),
...
def result_webhook(submission_uuid):
    ...
    count = database_util.command_execute(
        "SELECT COUNT(*) FROM `submission` WHERE solution_id=%s", (submission_uuid)
    )[0]["COUNT(*)"]
    if count == 0:
        return Response(
            json.dumps(
                error_dict(ErrorCode.INVALID_DATA, "submission_uuid not exist.")
            ),
            mimetype="application/json",
        )
    ...
    database_util.command_execute(
        ...
        (compile_result, compile_time, compile_memory, submission_uuid),
    )
    return Response(json.dumps({"status": "OK"}), mimetype="application/json")
