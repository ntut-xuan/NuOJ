...
from api.auth.google_oauth_util import google_login, _get_access_token_from_code, _get_user_email_from_access_token
...
def test_mock_user_profile_url_should_change_the_user_profile_url(app: Flask, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(api.auth.google_oauth_util, "USER_PROFILE_API_URL", "some_random_url")
...
class TestGoogleOAuthWithMOCK:
    def test_google_login_with_valid_code_should_passed_the_verify(self, app: Flask, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setattr(api.auth.google_oauth_util, "ACCESS_TOKEN_URL", "http://0.0.0.0:8080/test/google/access_token")
        monkeypatch.setattr(api.auth.google_oauth_util, "USER_PROFILE_API_URL", "http://0.0.0.0:8080/test/google/user_profile")
        with app.app_context():
    ...
    def test_google_login_with_valid_code_should_have_correct_user_email(self, app: Flask, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setattr(api.auth.google_oauth_util, "ACCESS_TOKEN_URL", "http://0.0.0.0:8080/test/google/access_token")
        monkeypatch.setattr(api.auth.google_oauth_util, "USER_PROFILE_API_URL", "http://0.0.0.0:8080/test/google/user_profile")
        with app.app_context():
    ...
    def test_google_login_with_valid_code_and_new_email_should_init_the_user_data(self, app: Flask, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setattr(api.auth.google_oauth_util, "ACCESS_TOKEN_URL", "http://0.0.0.0:8080/test/google/access_token")
        monkeypatch.setattr(api.auth.google_oauth_util, "USER_PROFILE_API_URL", "http://0.0.0.0:8080/test/google/user_profile")
        with app.app_context():
    ...
    def test_google_login_with_valid_code_and_new_email_should_init_the_user_profile_in_storage(self, app: Flask, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setattr(api.auth.google_oauth_util, "ACCESS_TOKEN_URL", "http://0.0.0.0:8080/test/google/access_token")
        monkeypatch.setattr(api.auth.google_oauth_util, "USER_PROFILE_API_URL", "http://0.0.0.0:8080/test/google/user_profile")
        with app.app_context():
    ...
    def test_google_login_with_invalid_code_should_fail_the_verify(self, app: Flask, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setattr(api.auth.google_oauth_util, "ACCESS_TOKEN_URL", "http://0.0.0.0:8080/test/google/access_token")
        monkeypatch.setattr(api.auth.google_oauth_util, "USER_PROFILE_API_URL", "http://0.0.0.0:8080/test/google/user_profile")
        with app.app_context():
