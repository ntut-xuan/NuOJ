...
from api.auth.github_oauth_util import (
    github_login,
...
def test_mock_access_token_url_should_change_the_access_token_url(
    ...
    monkeypatch.setattr(
        api.auth.github_oauth_util, "ACCESS_TOKEN_URL", "some_random_url"
    )
...
def test_mock_user_profile_url_should_change_the_user_profile_url(
    ...
    monkeypatch.setattr(
        api.auth.github_oauth_util, "USER_PROFILE_API_URL", "some_random_url"
    )
...
def test_validate_code_with_valid_oauth_code_should_return_valid_access_token(
    ...
    access_token: str | None = _validate_github_oauth_code_and_get_access_token(
        "some_client_id", "some_secret", "valid_code"
    )
...
class TestGithubOAuthWithMOCK:
    def test_github_login_with_valid_code_should_passed_the_verify(
    ...
    def test_github_login_with_valid_code_should_have_correct_user_email(
        self, app: Flask, monkeypatch: pytest.MonkeyPatch
    ):
        monkeypatch.setattr(
            api.auth.github_oauth_util,
    ...
    def test_github_login_with_valid_code_and_new_email_should_init_the_user_data(
        self, app: Flask, monkeypatch: pytest.MonkeyPatch
    ):
        monkeypatch.setattr(
            api.auth.github_oauth_util,
    ...
    def test_github_login_with_valid_code_and_new_email_should_init_the_user_profile_in_storage(
        self, app: Flask, monkeypatch: pytest.MonkeyPatch
    ):
        monkeypatch.setattr(
            api.auth.github_oauth_util,
