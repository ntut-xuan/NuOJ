...
class TestLoginUtil:
    ...
    def test_with_incorrect_password_should_return_false(
        self, app: Flask, setup_test_user: None
    ):
        with app.app_context():
...
class TestRegisterUtil:
    def test_register_account_should_write_data_into_database(self, app: Flask):
        with app.app_context():
            ...
            assert (
                user.password
                == "cc28a9d01d08f4fa60b63434ce9971fda60e58a2f421898c78582bbb709bf7bb"
            )
    ...
    def test_register_account_with_mail_verification_enabled_should_send_the_email(
        self, app: Flask, enabled_mail_setting: None
    ):
        with app.app_context():
            register("nuoj@test.com", "nuoj_test", "nuoj_test")
    ...
    def test_register_account_with_mail_verification_enabled_should_create_verification_code_to_app_config(
        self, app: Flask, enabled_mail_setting: None
    ):
        with app.app_context():
            register("nuoj@test.com", "nuoj_test", "nuoj_test")
...
class TestHandleUtil:
    def test_setup_handle_should_set_into_database(self, app: Flask):
        with app.app_context():
            user_without_handle: User = User(
                user_uid="DOES-NOT-MATTER-ID",
                handle=None,
                password="not-matter",
                email="nuoj@test.com",
                role=0,
            ...
            user_with_handle: User | None = User.query.filter(
                User.email == "nuoj@test.com"
            ).first()
            assert user_with_handle is not None
            assert user_with_handle.handle == "test-handle"

    def test_user_have_handle_is_already_have_handle_should_return_true(
        self, app: Flask
    ):
        with app.app_context():
            user_with_handle: User = User(
                user_uid="DOES-NOT-MATTER-ID",
                handle="nuoj",
                password="not-matter",
                email="nuoj@test.com",
    ...
    def test_user_not_have_handle_is_already_have_handle_should_return_false(
        self, app: Flask
    ):
        with app.app_context():
            user_with_handle: User = User(
...
class TestSendEmail:
    def test_send_email_to_fake_smtp_server_should_record_to_fake_smtp_server(
        self, app: Flask, setup_test_user: None, enabled_mail_setting: None
    ):
        with app.app_context():
