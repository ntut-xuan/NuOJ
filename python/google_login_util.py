...
def googleLogin(args, settingJsonObject):
    if args.get("error"):
        ...
        data = {
            "status": "OK",
            "result": {"code": args.get("code"), "scope": args.get("scope")},
        }
    ...
    post_data = {
        ...
        "redirect_uri": "https://nuoj.ntut-xuan.net/google_login",
    ...
    result = database_util.command_execute(
        "SELECT COUNT(*) FROM `user` WHERE email=%s", (email)
    )[0]
    count = result["COUNT(*)"]
    ...
    if count == 0:
        ...
        database_util.command_execute(
            "INSERT INTO `user`(user_uid, password, email, role, email_verified) VALUES(%s, %s, %s, %s, %s)",
            (user_uid, str(uuid4()), email, 0, True),
        )
        database_util.command_execute(
        ...
        if not database_util.file_storage_tunnel_exist(
            ...
            database_util.file_storage_tunnel_write(
                user_uid + ".json",
                json.dumps({"handle": "", "email": email, "school": "", "bio": ""}),
